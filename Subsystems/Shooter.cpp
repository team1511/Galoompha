...
void ShooterWheel::setTargetSpeed(float speed) {
	...
	target = speed;
	double actual = encoder->PIDGet();
	double error_new = target - actual;
	double diff = error_new - error_last;
	integral += error_new;
	double output = (f*target) + (p * error_new) + (d * diff) + (i * integral);
	error_last = error_new;
	
	
	shooterWheel->Set(-output);

	printf("%f %f %f error %f\n", target, actual, output, error_new);
